...
def acc(data_source):
    ...
    return correct / len(data_source)
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            with open ('log.txt', 'a') as f:
                ...
                elapsed * 1000 / args.log_interval, cur_loss), file=f)
            total_loss = 0
            start_time = time.time()
        if args.dry_run:
            break
...
try:
    for epoch in range(1, args.epochs + 1):
        ...
        val_acc = acc(val_data)
        with open ('log.txt', 'a') as f:
            print('-' * 89, file=f)
            print('| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.2f} | '
                    'valid acc {:8.4f}'.format(epoch, (time.time() - epoch_start_time),
...
test_acc = acc(test_data)
with open ('log.txt', 'a') as f:
    print('=' * 89, file=f)
    print('| End of training | test loss {:5.2f} | test acc {:8.4f}'.format(
